...
export default function ScheduleList({ schedules, onScheduleClick, loading }: ScheduleListProps) {
  if (loading) {
  ...
  if (schedules.length === 0) {
    return (
      <Card padding="lg" variant="subtle" className="border-dashed border-2 bg-transparent text-center opacity-70 py-16">
      ...
      {schedules.map((schedule) => (
        ...
        >
          ...
          <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto mt-2 sm:mt-0">
            ...
            <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full mt-1 tracking-tighter shadow-sm ${
              schedule.status === 'paid' ? 'bg-green-100 text-green-700' : 
              schedule.status === 'missed' ? 'bg-red-100 text-red-700' :
              schedule.status === 'partial' ? 'bg-orange-100 text-orange-700' :
