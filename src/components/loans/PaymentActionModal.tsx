...
import { CheckCircle2, PieChart, TrendingUp, XCircle, AlertCircle } from 'lucide-react';
...
export default function PaymentActionModal({
  ...
  const [amountPaid, setAmountPaid] = useState<string>(defaultAmountDue.toString());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (actionType === 'partial') {
        if (finalAmount <= 0) throw new Error('Partial payment must be greater than 0');
        if (finalAmount >= defaultAmountDue) throw new Error('Partial payment must be less than the total due. Use "Full Payment" instead.');
      }

      if (actionType === 'advance') {
        if (finalAmount <= defaultAmountDue) throw new Error('Advance payment must be greater than the current installment.');
      }
  ...
  const actions = [
    { id: 'paid', label: 'Full Payment', icon: CheckCircle2, color: 'text-green-600', bg: 'bg-green-50', hover: 'hover:border-green-400 hover:shadow-green-100', border: 'border-green-500', activeBg: 'bg-green-500 text-white' },
    { id: 'partial', label: 'Partial', icon: PieChart, color: 'text-orange-500', bg: 'bg-orange-50', hover: 'hover:border-orange-400 hover:shadow-orange-100', border: 'border-orange-400', activeBg: 'bg-orange-500 text-white' },
    { id: 'advance', label: 'Advance', icon: TrendingUp, color: 'text-blue-600', bg: 'bg-blue-50', hover: 'hover:border-blue-400 hover:shadow-blue-100', border: 'border-blue-500', activeBg: 'bg-main-blue text-white' },
    { id: 'absent', label: 'Missed', icon: XCircle, color: 'text-red-500', bg: 'bg-red-50', hover: 'hover:border-red-400 hover:shadow-red-100', border: 'border-red-500', activeBg: 'bg-red-500 text-white' },
  ] as const;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <Card variant="elevated" padding="lg" className="w-full max-w-[420px] bg-white relative z-10 shadow-2xl rounded-3xl border border-gray-100 overflow-hidden transform transition-all flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 overflow-y-auto pr-1 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-3">
            {actions.map((act) => {
              ...
              return (
                ...
                >
                  <Icon size={28} strokeWidth={isActive ? 2.5 : 2} className={isActive ? 'text-white' : act.color} />
                  <span className={`text-xs font-bold uppercase tracking-wider ${isActive ? 'text-white/90' : 'text-gray-500'}`}>
                    {act.label}
                  </span>
                </button>
              );
          ...
          <div className="flex flex-col gap-4 transition-all">
            ...
            {(actionType === 'partial' || actionType === 'advance') && (
              <div className="flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
                <label className="text-[11px] font-black uppercase text-gray-400 tracking-widest pl-1">Amount Given (₱)</label>
            ...
            <div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100 flex gap-3 items-start">
              <div className="mt-0.5 text-main-blue"><CheckCircle2 size={16} /></div>
          ...
          <div className="mt-2 flex gap-3 pt-4 border-t border-gray-100">
            <Button variant="outline" size="lg" onClick={onClose} className="w-1/3 mt-0 text-gray-500 border-gray-200 hover:bg-gray-50">
              Cancel
            </Button>
            <Button variant="blue" size="lg" type="submit" disabled={loading} className="w-2/3 mt-0 shadow-lg shadow-blue-500/20">
